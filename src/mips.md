...
reg: DIVI(reg,reg)  "jal $div\n"   1
reg: DIVU(reg,reg)  "jal $divu\n"  1
reg: MODI(reg,reg)  "jal $rem\n"   1
reg: MODU(reg,reg)  "jal $remu\n"  1
...
static void target(p) Node p; {
	assert(p);
	switch (p->op) {
	case MULI: case MULU: case MODI: case MODU: case DIVI: case DIVU:
		setreg(p, ireg[2]);
		rtarget(p,0,ireg[4]);
		rtarget(p,1,ireg[5]);
		break;
